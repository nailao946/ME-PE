...
| 📅 日历 | 月视图完成率色块、当日任务详情、今日任务状态/连续打卡统计 |
...
- **日历和目标统计按日期过滤**：只列出当天实际出现的任务，未设置每日目标的量化任务不参与统计；目标页「今日完成任务」改为当天应做任务中的完成数
- **日历统计调整**：取消「月度打卡率」，改为「今日任务：已完成/未完成」；连续打卡改为每天至少完成一个任务即算成功，无任务日不打断
- **量化每日目标改为增量口径**：累加模式按“今日值 - 今日首次操作前基线 ≥ 每日目标”判定，更新模式保留绝对值口径；跨日自动保存基线，漏掉的日期不回溯补记
- **云同步反馈与冲突处理增强**：上传/下载逐一显示 GitHub、Gitee、WebDAV 成功或未成功及原因；计时、专注、打卡、健康、喝水记录新增 Uid，双端同时产生时自动合并，避免整文件覆盖丢失
- **Gitee/WebDAV 稳定性增强**：Gitee 限流自动退避重试并给出明确提示，WebDAV 网络请求自动重试，完整同步保留上传与下载的多云端明细
